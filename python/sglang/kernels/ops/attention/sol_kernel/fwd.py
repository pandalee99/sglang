...
def _validate(q, k, v, kv_splits, thresh_type):
    ...
    allowed = ((8, 6), (8, 9), (9, 0), (10, 0), (12, 0), (12, 1))
    # LOCAL EDIT to a verbatim vendored file (backup: fwd.py.orig_backup).
    # SM103 (B300) is absent upstream, and the docstring above deliberately does not
    # treat unlisted archs as implicitly compatible. Kept behind an explicit opt-in
    # rather than added to the tuple, so the default stays exactly upstream's.
    # Justification is measured, not architectural: _use_pointer_arch() puts (10, 3)
    # on the same TMA path as (10, 0), and on that path this kernel ran 1.46x/1.95x
    # against dense on H3's real shapes with cos_sim 0.99+ on smooth inputs.
    if os.environ.get("SGLANG_SOL_ATTN_ALLOW_SM103", "0") not in ("0", ""):
        allowed = allowed + ((10, 3),)
